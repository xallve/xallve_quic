...
def run_client(host='localhost', port=4433, encryption_key=None):
    ...
    def retransmission_thread(retransmission_handler):
        while not retransmission_handler.stop_event.is_set():
    ...
    threading.Thread(target=retransmission_thread, args=(retransmission_handler,), daemon=True).start()
        ...
        try:
            ...
            if packet.packet_type == HANDSHAKE_PACKET:
                ...
                if packet.payload == b'ACK':
                    retransmission_handler.stop_event.set()
